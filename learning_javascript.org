...
The parent of object created by =new <F>(...)= constructor is:

+ the value of =<F>.prototype= at the time =new <F>(...)= is called, if the
value returned by the function is not an object. For example:
...
#+begin_src javascript
  ...
  FF["prototype"] = o1;
  var ff = new FF();
  console.log(Object.getPrototypeOf(ff) === o1); // true
#+end_src

+ if the function has a =return= statement and returns an object, then parent
is whatever the returned object's parent is.
