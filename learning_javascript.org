...
+ If the object has a =valueOf()= method that returns a primitive value,
  JavaScript converts that primitive value to a number and ruturns the result.
...
all objects that are not dates, and an bject-to-string conversion for Date
...
funciton are visible /throughout/ the body of the function. Curiously, this
means that varibles are even visible before they are declared.
...
language, it is a good habit to declare all varibles at the top of the function
which accurately reflect the true scope of the varaibles.
...
Two great article on execution context and scope chain. [From David Sherif]
Links:
